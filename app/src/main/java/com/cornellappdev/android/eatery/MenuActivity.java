...
public class MenuActivity extends AppCompatActivity implements MessengerDialog.OnCompleteListener {
  TextView cafeText;
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    cafeName = (String) intent.getSerializableExtra("locName");
    ...
    fbMessengerButton = findViewById(R.id.fb_messenger);
    fbMessengerButton.setOnClickListener(new View.OnClickListener() {
      @Override
      public void onClick(View v) {
        startIntents();
      }
  ...
  public void onComplete(ArrayList<String> selected) {
    String str1 = "I'm hungry. Let's eat at " + cafeName + " today!";
    String str2 = "";
    for (String s : selected) {
      str2 = str2 + "\n" + s.trim();
    }

    str2 = "I want to try their " + str2;
    String finalStr = str1 + "\n\n" + str2;
    shareOnFacebookMessenger(finalStr);
  }

  private void startIntents() {
    String str1 = "I'm hungry. Let's eat at " + cafeName + " today!";
    if (cafeData instanceof DiningHallModel) {
      MealMenuModel mmm = ((DiningHallModel) cafeData).getCurrentDayMenu().getMeal(MealType.DINNER).getMenu();
      MessengerDialog md = new MessengerDialog();
      Bundle bundle = new Bundle();
      bundle.putSerializable("allItems", mmm.getAllItems().toArray(new String[mmm.getAllItems().size()]));
      md.setArguments(bundle);
      md.show(getSupportFragmentManager(), "missiles");
    } else {
      shareOnFacebookMessenger(str1);
    }
  }

  private void shareOnFacebookMessenger(String finalStr) {
    Intent sendIntent = new Intent();
    sendIntent.setAction(Intent.ACTION_SEND);

    sendIntent.putExtra(Intent.EXTRA_TEXT, finalStr);
    ...
    try {
      ...
      Toast.makeText(getApplicationContext(),"Please install facebook messenger.",
